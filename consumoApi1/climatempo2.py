...
if iTIPOCONSULTA == 3:
    ...
    for iCHAVE in iRETORNO_REQ['data']:
        ...
        print("data: " + str(iDATA) + " chuva: " + str(iCHUVA) + "%" + " temp: min(" + str(
            iTEMPERATURAMIN) + ") max(" + str(iTEMPERATURAMAX) + ") resumo: " + str(iTEXTMORNING) + "\n")

# 4=Previsao para os proximas 3 dias por regiao
if iTIPOCONSULTA == 4:
    iURL = "http://apiadvisor.climatempo.com.br/api/v1/forecast/region/nordeste?token=" + iTOKEN
    iRESPONSE = requests.request("GET", iURL)
    iRETORNO_REQ = json.loads(iRESPONSE.text)
    # print(iRETORNO_REQ)
...
if iTIPOCONSULTA == 6:
    ...
    for iCHAVE in iRETORNO_REQ:
        ...
        print("id:" + str(iID) + " - " + "stlocalesidate:" + str(iSTATE) + " - " + "country:" + str(
            iCOUNTRY) + " - " + "name:" + str(iNAME) + "\n")
